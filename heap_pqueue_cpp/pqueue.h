...
private:
    ...
    ******************************************/
    ...
    // helpers for recursive traversal while merging
    Node* merge(Node* heapRoot1, Node* heapRoot2);
    Node* mergeSwap(Node* heapRoot1, Node* heapRoot2);
    // swap children of a node
    void swapChildren(Node* node);
    // copy a queue (from a Node pointer)
    Node* copyQueue(Node* node1, const Node* node2);
    // helper for recersive traversal while printing
    void printPatientQueue(Node* node) const;
